#Data Science Project - Census Income


censusData = read.csv('C:/Users/SAXON/Downloads/census-income.csv', stringsAsFactors = T )
View(censusData)
...
count_workclass = count(censusData,workclass)
count_workclass
...
ggplot(censusData,aes(x = relationship,fill = sex)) + geom_bar(position = "dodge") +
  labs(x = "Categories of Relationship" , y = "Count of categories", title = "Distribution of Relationship by Sex")
...
split_data = sample.split(censusData$hours.per.week, SplitRatio = 0.7)
censusTrain = subset(censusData,split_data == T)
censusTest = subset(censusData,split_data == F)

#ii.Build a linear model on the train set where the dependent variable is "hours.per.week" and independent variable is "education.num".
str(censusData)
...
split_data = sample.split(censusData$hours.per.week, SplitRatio = 0.65)
...
range(pred_val)
#iV) Plot Acuuracy vs cut-off and pick an ideal value for cut-off
library(ROCR) 
...
class_prediction = predict(census_model,newdata = censusTest, type = "class")
...
census_model = randomForest(formula=X~.,data=censusTrain,ntree=300)

# iii)	Predict values on the test set
cenus_prediction = predict(census_model,newdata = censusTest,type = "class")
